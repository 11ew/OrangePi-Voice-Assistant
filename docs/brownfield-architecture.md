...
- **音频配置**: `docs/AUDIO_SETUP.md` - 音频设备配置教程
...
| **VAD** | Silero VAD (sherpa-onnx) | latest | ❓ 待验证 | 模型可能未下载 |
| **ASR** | Paraformer INT8 | latest | ❓ 待验证 | 模型可能未下载 |
| **LLM** | DeepSeek API | deepseek-chat | ✅ 已验证 | 平均响应 2.2秒 |
| **TTS** | Edge-TTS | ≥6.1.0 | ✅ 已验证 | 平均生成 2.7秒 |
| **音频捕获** | ALSA (arecord) | v1.2.6 | ✅ 已验证 | 16kHz 单声道 |
...
│   ├── audio_record.py         # 录音脚本 ❓ 待验证
│   ├── speech_to_text.py       # ASR 识别脚本 ❓ 待验证
...
│   ├── vad_detector.py         # VAD 检测器 ❓ 待验证
│   ├── asr_engine.py           # ASR 引擎 ❓ 待验证
...
**文档创建者：** 哈雷酱（傲娇大小姐工程师）
**最后更新：** 2026-01-18 16:45
**文档状态：** v1.1 - Phase 1 完成后更新
